...
class Board:
    ...
    def _from_dict(self, data: dict):
        ...
        gdata = data["graph"]
        self.graph = Graph(
            n=len(gdata["vertices"]), edges=gdata["edges"], directed=False)
    ...
    def _discard_disconnecting_fences(self):
        for placement in self.valid_fence_placements:
            ...
            pawn_membership = membership[graph_copy.vs.find(
                name=self.pawns[self.curr_player - 1])]
    ...
    def update(self, action: Action):
        if isinstance(action, MovePawn):
            ...
            if (x, y) not in self.get_valid_pawn_moves():
                raise ValueError(f"Invalid pawn move to {x}, {y}")
            self.move_pawn(x, y)

        elif isinstance(action, PlaceFence):
            x, y, direction = action.x, action.y, action.direction
            if (x, y, direction) not in self.get_valid_fence_placements():
                raise ValueError("Invalid fence placement")
            self.place_fence(x, y, direction)

        # WIN CONDITION LOGIC
        # Player 1 (Red) starts at y=1, wins at y=9
        # Player 2 (Blue) starts at y=9, wins at y=1
